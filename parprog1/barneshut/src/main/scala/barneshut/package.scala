...
package object barneshut {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val bx = if (b.x > boundaries.maxX) boundaries.maxX
               else if (b.x < boundaries.minX) boundaries.minX
               else b.x
      val by = if (b.y > boundaries.maxY) boundaries.maxY
               else if (b.y < boundaries.minY) boundaries.minY
               else b.y

      val x = ((bx - boundaries.minX) / sectorSize).toInt
      val y = ((by - boundaries.minY) / sectorSize).toInt
