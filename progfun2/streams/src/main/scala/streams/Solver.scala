...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
      def f(x: List[(Block, Move)], history: List[Move]): Stream[(Block, List[Move])] =
        x match {
            case Nil => Stream.empty
            case (next, move) :: other => Stream.cons((next, move :: history), f(other, history))
        }
      f(b.legalNeighbors, history)
  }
