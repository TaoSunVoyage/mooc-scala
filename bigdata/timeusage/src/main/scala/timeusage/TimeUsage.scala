...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType((0 until columnNames.length) map {
      case 0 => StructField(columnNames(0), StringType, false)
      case i if i > 0 => StructField(columnNames(i), DoubleType, false)
    })
  ...
  def row(line: List[String]): Row =
    Row.fromSeq(line.zipWithIndex.map{case (l, i) => if (i==0) l else l.toDouble})
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    columnNames.foreach {c => 
      if (primaryNeedsPrefix.exists(c.startsWith)) primaryNeedsColumns = primaryNeedsColumns :+ col(c)
      else if (workPrefix.exists(c.startsWith)) workColumns = workColumns :+ col(c)
      else if (otherPrefix.exists(c.startsWith)) otherColumns = otherColumns :+ col(c)
    }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed
          ...
          .agg(round(mean($"primaryNeeds"), 1).as("primaryNeeds"), 
