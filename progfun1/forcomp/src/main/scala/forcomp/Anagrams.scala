...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(x => x).toList.map(x => (x._1, x._2.length)).sortBy(_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
   ...
   //   val ymap = y.toMap
   //
   //   (for (k <- xmap.keys) yield
   //       if (ymap.contains(k)) (k, xmap(k) - ymap(k))
   //       else (k, xmap(k))
    ...
    def loop(occ: Occurrences): List[Sentence] =
      if (occ.isEmpty) List(List())
        ...
        for {
          ...
          other <- loop(subtract(occ, combination))
        } yield word :: other
      }
